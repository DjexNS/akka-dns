...
import akka.io.AsyncDnsResolver.{NSResolved, SrvResolved}
...
class AsyncDnsResolverSpec extends AkkaSpec(
  """
    ...
    akka.io.dns.async-dns.nameservers = ["8.8.8.8", "8.8.4.4", "9.9.9.9"]
    akka.io.dns.async-dns.resolve-ns = true
    akka.io.dns.async-dns.resolve-srv = false
  """) {
...
//      answer.ipv6.toSet should be(empty)
//    }
    "resolve NS records" in {
      val answer = Await.result(IO(Dns) ? Dns.Resolve("bbc.co.uk"), duration).asInstanceOf[NSResolved]
      answer.name should equal("bbc.co.uk")
      answer.ns.head.name should equal("bbc.co.uk")
      answer.ns.map(el => el.nsName) should (contain("ns3.bbc.net.uk") and have length 4)
    }
